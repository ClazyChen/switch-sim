...
namespace fpga::p4::mat::hash {
    ...
    private:
        ...
        template <size_t I, size_t J> requires (0 <= J && J < value_width)
            void init_matrix() {
            auto unit = Hex::from<&Program<mau_id>::mat::hash::matrix[I * value_width + J]>();
            matrices[I][J] = 0;
        ...
        template <size_t I> requires (0 <= I && I < hash_count)
            void init() {
            masks[I] = Hex::from<&Program<mau_id>::mat::hash::mask[I]>();
            salts[I] = Hex::from<&Program<mau_id>::mat::hash::salt[I]>();
            init_matrix<I, 0>();
